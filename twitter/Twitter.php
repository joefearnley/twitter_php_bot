...
class twitter {
	 ...
	 * @return string
	 */
	function showFriendship( $source_user_id,  $target_user_id)
	{
	    if( !in_array( $this->type, array( 'xml','json' ) ) )
	        return false;

		$qs = '?source_id=' . rawurlencode( $source_user_id ) . '&target_id=' . rawurlencode( $target_user_id);
	...
	/**
	 * Returns extended information of a given user, screen name as per the required
	 * id parameter below.  This information includes design settings, so third party developers can theme
	 * their widgets according to a given user's preferences.	 
	 *
	 * @author joe fearnley
